...
<!-- CONTENT -->
# Logistisk regression {#sec-logistic}
...
### Logistiska funktionen {#sec-logistic-func}
...
## Maximum likelihoodmetoden {#sec-maximum-likelihood}
...
En likelihoodfunktion är en funktion som givet parametervärden visar hur troligt det observerade data är. När likelihoodfunktionen är som störst indikerar den på parametrarna som är mest trolig att ha genererat de observerade data, det vill säga optimeringen vill maximera funktionen iterativt. Vanligtvis optimeras **loglikelihood**-funktionen då transformationen till log ger enklare ekvationer av derivatorer för optimeringsalgoritmerna.

Vi kan visa processen med ett enkelt exempel där vi har en analytisk lösning. För en Bernoulli-fördelad slumpvariabel kan vi skriva loglikelihoodfunktionen som: 
$$
\begin{aligned}
  \ln L(\pi) &= log\left[\prod\limits_{i = 1}^n \left( \pi^{Y_i} \cdot (1 - \pi)^{1 - Y_i}\right) \right] 
  \\&= \sum\limits_{i = 1}^{n_1}\ln(\pi) + \sum\limits_{i = n_1 + 1}^{n}\ln(1 - \pi) \\
  &= n_1 \cdot \ln(\pi) + (n - n_1) \ln (1 - \pi)
\end{aligned}
$$

där $\pi$ är sannolikheten för $P(Y=1)$, $n$ är antalet observationer och $n_1$ är antalet 1:or i materialet^[Vi antar att vi summerar uttrycket först över alla 1:or med tillhörande sannolikhet $\pi$.]. 

Nästa steg i optimeringen är att derivera uttrycket.

$$
\begin{aligned}
\frac{d}{d\pi}logL(\pi) &= n_1 \cdot \frac{1}{\pi} + (n - n_1) \frac{1}{1 - \pi} \cdot -1 \\
&= \frac{n_1}{\pi} - \frac{n - n_1}{1 - \pi}
\end{aligned}
$$
och till sist identifiera när derivatan är lika med 0. 

$$
\begin{aligned}
\frac{d}{d\pi}logL(\pi) = 0 &= n_1 \cdot (1 - \pi) - (n - n_1) \cdot \pi\\
0 &= n_1 - n\pi \\
\pi &= \frac{n_1}{n}
\end{aligned}
$$
I detta enkla fall med den analytiska lösningen får vi fram en enkel formel som visar på den skattning som ger det största värdet på likelihoodfunktionen. I mer komplexa fall behöver vi använda någon form av iterativ optimering som successivt förflyttar sig i derivatans riktning tills ett optimum nås.

Följande visualisering visar hur ett slumpmässigt valt startvärde (rödstreckad linje) stegvis flyttar sig tills vi når ett optimum (svartstreckade linje).
```{r}
#| fig-width: 5
#| fig-height: 3
...
När vi modellerar den Bernoullifördelade variabeln med en logistisk regression antar vi att $P(Y = 1)$ varierar beroende på värden av våra förklarande variabler, så vi byter ut $\pi$ med ett uttryck som förhåller sig till de förklarande variablerna och $\beta$. Detta ger loglikelihoodfunktionen: 
$$
\begin{aligned}
\ln L(\boldsymbol{\beta}) = \sum_{i = n_1 + 1}^{n} \operatorname{-}\mathbf{X}_i'\boldsymbol{\beta} - \sum_{i=1}^n \ln (1 + exp\left[\operatorname{-}\mathbf{X}_i'\boldsymbol{\beta} \right])
\end{aligned}
$$
Denna funktion optimeras numeriskt, vanligtvis med Fisher Scoring Algorithm^[En algoritm som använder andraderivator för att succestivt hitta bättre värden för $\boldsymbol{\beta}$. För mer detaljer, se följande länk: https://en.wikipedia.org/wiki/Scoring_algorithm.]. Det är denna algoritm som R använder sig utav när en logistisk regressionsmodell anpassas.
