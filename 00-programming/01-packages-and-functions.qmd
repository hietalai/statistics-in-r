...
# Att använda R {#sec-using-R}
Det kommer bli väldigt jobbigt om vi alltid måste skapa data för hand eller beräkna summor utifrån enskilda objekt eller element, `a + b + c + ...`. Användandet av en programvara ska underlätta de statistiska beräkningarna och i den andan följer här några kapitel om hur R eller RStudio kan hjälpa till att bearbeta och analysera information.

## Paket och funktioner
Inom programmering är det vanligt att man genomför samma komplexa operation flera gånger och ett enkelt sätt att underlätta kodningen är att skapa en **funktion** som förkortar flera rader kod till en. Flera liknande funktioner samlas i olika **paket** som vi antingen har skapat själv, hittat i paket som redan finns med standardinstallationen av R eller laddat hem från internet.

:::{.callout-important}
Paket som inte finns med i standardinstallationen av R måste först **installeras** för att kunna användas. Installera betyder i detta ändamål att ladda hem och göra tillgängligt paketet för användning i R. Detta kan låta komplicerat men i 99% av fallen behöver vi inte göra mer än att använda funktionen `install.packages("PAKETNAMN")`. Notera användningen av `"` i funktionen för att inuti ange det namn som paketet har.
:::
...
Skillnaden här är att namnet på paketet inte ska anges inom citationstecken, ex. `PAKETNAMN`, istället för inom som vi gjorde i `install.packages()`^[Detta beror på att R använder paket som objekt vilket identifieras utan citationstecknen]. Om du en gång redan installerat paketet till din version av R behöver du **inte installera paketetet för varje ny session** utan kan direkt ladda det genom `library()` eller `require()`.

```{r}
#| eval: false

## Om paketen inte finns på datorn måste de installeras. 
## KÖRS ENDAST EN GÅNG I SAMBAND MED EN NY INSTALLATION ELLER UPPDATERING AV R-versionen!
...
Denna kod kör funktionen `funktion` där `argument1` antar värdet `värde1` och `argument2` antar värdet `värde2`. 
...
R fortsätter att läsa på nästa rad om programmet inte anser att raden avslutats, i detta fall med att funktionen avslutas med `)`. Mer specifikt kan vi läsa av raderna en och en likt:
...
Den enklaste formen av fil som R läser in är rena textfiler (med filändelser `.txt`, `.csv`, `.dat`) som på olika sätt sparat ner datans struktur på ett simpelt och organiserat sätt. Vi kan läsa in informationen från dessa sorters filer till R med hjälp av specifika funktioner. 

Inom statistik är det vanligt att använda kommaseparerade filer, `.csv`, som vi kan skapa från Excel genom menyn `Spara som...` och ange `.csv` som filformat. Med hjälp av funktionen `read.csv` och dess argument kan vi säga till R vilken struktur som filen har och läsa in samma struktur som vi hade i Excel i en data frame. 

:::{.callout-tip}
Ett tips är att öppna datafilen i **Notepad**, **Anteckningar** eller annan enkel ordbehandlare för att se vilka symboler som används för att beskriva strukturen. Detta är speciellt viktigt för `.csv` filer då svenska standarder inte använder `,` som kolumnavskiljare utan istället `;`. Varför detta är så ligger utanför detta material. 
:::
...
I ovanstående exempel får vi nu ett objekt `ekar` som innegåller fyra kolumner med information. Vi ser också i kodexemplet hur en uppdelning på flera rader ger oss möjligheten att kommentera respektive argument.

## Arbetsmappar
Att importera datamaterial är endast ett sätt att interagera med datorns hårddisk. Vi kan ibland vilja spara ner objekt eller visualiseringar mer permanent och då kommer vi återigen behöva ange en **sökväg** till någonstans på datorn. För att underlätta denna process kan vi använda oss utav **arbetsmappar** (eng. working directory).
...
- [instruktioner för Linux](https://askubuntu.com/questions/1528790/how-to-copy-file-path-to-clipboard){target="_blank"}
...
```{r}
#| eval: false
## Arbetsmappen anger en sökväg till en mapp på datorn med alla 
## filer som ska användas 
...
:::{.callout-tip}
RStudio har ett alternativt sätt att använda specifika delar av hårddisken för olika ändamål. Ett **Projekt** kan skapas som säger åt R att arbeta inom den mapp som projektet finns i. Ni kan läsa mer om denna funktion [här](https://support.posit.co/hc/en-us/articles/200526207-Using-RStudio-Projects).
:::


## Hjälpdokumentation {#sec-help}
...
:::{.callout-tip}
Du kan också använda kortkommandot `F1` på tangentbordet när du markerat en funktion för att direkt komma till dess dokumentation.
:::

Eftersom R är open-source finns det väldigt mycket material tillgänglig på internet, detta underlag kan ses som ett exempel på en sådan resurs. Genom att använda en sökmotor där man beskriver det problem som man stött på eller fråga hur en funktion fungerar kommer du hitta en stor samling av sidor som säkerligen beskriver exakt det du är ute efter. 

### ChatGPT eller annan AI {#sec-chat-GPT}
