...
Detta är egentligen första gången ett sådant krav explicit uppkommer när vi har att göra med tidsserier så ett alternativt sätt att formulera PAC utan detta krav är:
$$
  \begin{aligned}
  ...
  \end{aligned}
$$ {#eq-pacf}
där $\beta_1$ till $\beta_{k-1}$ är parametrar i en linjär prediktor som anpassats för att minimera MSPR till $Y_t$ och $Y_{t-k}$ respektive. PAC blir då med denna formulering korrelationen mellan prediktionsfelet från de två tidpunkterna.

Vi anser $\phi_{11} = 1$ och kan visa att $\phi_{22}$ beräknas enligt:
$$
  \begin{aligned}
...
$$
  ...
  \end{aligned}
$$
det vill säga ytterligare ett lagg $\ne 0$. 

En generell regel för AR(p) är att för större lagg gäller $\phi_{kk} = 0$ när $k > p$. Detta innebär att vi får samma funktionalitet i den partiella autokorrelationen för en AR-process som vi har i autokorrelationen för en MA-process; antalet nollskilda lagg motsvarar ordningen för processen.

Den partiella autokorrelationen kan också visas för en MA-process. För MA(1) gäller enligt @eq-pacf22:
...
tibble(
  ...
  `AR(p)` = c("Långsamt avtagande", "0 efter lag p"),
  `MA(q)` = c("0 efter lag q", "Långsamt avtagande")
) |>
  kable(
